			...
			if (httpRequest.getPath().getValue().endsWith("/api/v2/status")) {
				...
				{	
					...
					response = client.sendRequest(HttpRequest.request().withMethod("GET")
							...
							.withPath("/api/v2/status")).get().getBodyAsRawBytes();
					
					if(TestProperties.STORE)
						Files.write(Paths.get(TestProperties.STORE_PATH + File.separator +
								TestProperties.CBOR_STATUS_RESPONSE_FILE), response);
					
				}
				else
					response = Files.readAllBytes(Paths.get(
							getClass().getClassLoader().getResource(TestProperties.CBOR_STATUS_RESPONSE_FILE).getPath()));
